...
iterations = 30*92  # number of training iterations
...
print_freq = 46  # print training status once every __ batches
...
valid_after_epoch = 7  # No of epochs to run before making a validation run
...
def main_srgan():
    ...
    # Epochs
    no_epoch = 1
    for epoch in tqdm(range(start_epoch, epochs), desc="Training Network epochs", unit=' epoch',
                      total=(epochs - start_epoch), position=0):

        # At the quarter points, reduce learning rate to half each
        if (epoch == int((3 * iterations / 4) // len(train_loader) + 1)) or \
                (epoch == int((2 * iterations / 4) // len(train_loader) + 1)) or \
                (epoch == int((1 * iterations / 4) // len(train_loader) + 1)):
        ...
        if no_epoch % 5 == 0:
            torch.save({'epoch': epoch,
                        ...
                        'optimizer_g': optimizer_g,
                        'optimizer_d': optimizer_d},
                       f'checkpoint_srgan.pth (version3 {epoch}).tar')

        if no_epoch % valid_after_epoch == 0:
            valid(valid_loader=valid_loader,
                  generator=generator)
        no_epoch += 1


def valid(valid_loader, generator):
    generator.eval()
    with torch.no_grad():
        # calculate metrices:
        metrics = {'rmse': 0, 'sre': 0, 'uqi': 0}
        for i, (lr_img, hr_img) in tqdm(enumerate(valid_loader), desc="Validation", unit=' batch',
                                        position=1, leave=False, total=len(valid_loader)):
            # Move to device
            lr_img = lr_img.to(device)

            # Forward prop.
            sr_img_srgan = generator(lr_img)
        ...
        print("")
        for m in metric.keys():
            metrics[m] /= len(valid_loader)
            print(f"Average metric in validation: {m} -> {metrics[m]}")
...
def train(train_loader, generator, discriminator, truncated_vgg19, content_loss_criterion, adversarial_loss_criterion,
    ...
    # Batches
    for i, (lr_imgs, hr_imgs) in tqdm(enumerate(train_loader), desc="Training one epoch", unit=' batch',
